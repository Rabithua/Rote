...
| 路径 | 方法 | 认证 | 描述 |
| --- | --- | --- | --- |
| `/notes/:id/share` | GET | 作者 JWT | 返回当前链接的 `{ token, createdAt }`，未开启时为 `null` |
| `/notes/:id/share` | PUT | 作者 JWT | 创建链接或返回已有链接；无需请求体，重复调用不轮换令牌 |
| `/notes/:id/share` | DELETE | 作者 JWT | 停止分享，返回 `null`；重复调用成功 |
| `/shares/:token` | GET | 无 | 读取笔记最新内容、附件、作者展示信息和当前关联文章全文 |

Web 分享地址为 `/s/:token`。每篇笔记最多一个有效链接，令牌是 32 字节安全随机数的 Base64URL 编码。链接默认长期有效，可转发；停止分享后再次开启会生成新令牌。分享管理仅向作者开放，其他用户得到 404，未认证请求得到 401。

分享与 `private / public` 独立：开启链接不会公开笔记，归档和可见性切换不撤销链接，删除笔记则级联撤销。无效、已撤销或关联笔记已删除的链接统一返回 404 和 `share_not_found`。服务端内部错误返回 500 和 `share_request_failed`，不回显令牌。

匿名读取只返回展示字段，不返回笔记 ID、文章 ID、邮箱、反应记录、内部存储字段或分享管理数据。链接不授予普通笔记/文章接口、Open API、MCP 或写操作的权限；私密笔记仍不会进入公开搜索、Explore 或 RSS。页面展示最新内容，不创建快照。

分享响应使用 `Cache-Control: no-store`，Web 页面禁止索引且不发送 Referer。应用日志遮蔽分享令牌，内置 Nginx 关闭 `/s/` 访问日志；自托管时的额外代理或统计服务也应避免记录完整分享链接。

附件沿用当前存储 URL：停止分享会阻止新的分享内容请求，但已经取得的附件直链、已下载的内容不会被撤回。公开笔记停止链接分享后，原有公开地址仍然有效。
