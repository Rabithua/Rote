...
如需重新构建，请使用 `重建向量索引`。被替代代次的向量、任务和 HNSW 索引会在新代次成功就绪后回收；重建失败时保留。没有代次的历史向量仍保留，供明确恢复或追溯。只有确实需要删除全部向量时才执行 `清空索引`，该操作也会移除代次 HNSW 索引，但不会删除原始笔记或文章。
...
没有代次的旧向量保留用于追溯，不参与当前检索。已有代次在配置切换后仍保留；明确重建并成功就绪后，被替代代次的向量、任务和物理索引才会回收。明确执行“清空索引”会删除所有向量、队列任务和代次 HNSW 索引，并要求重建。切勿通过回退应用版本直接使用升级后的数据库；如需整体回滚，应同时恢复升级前的应用版本和数据库备份。
