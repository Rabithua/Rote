...
async function processDirectory(dir: string): Promise<void> {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      // 如果是图片文件，进行压缩
      const targetFilePath = path.join(targetPath, `${path.parse(entry.name).name}.webp`);
      ...
      try {
        ...
        if (error.message.includes('unsupported image format')) {
          ...
          stats.unsupported[fileExtension] = (stats.unsupported[fileExtension] || 0) + 1;
        } else {
...
processDirectory(sourceDir)
  .then(() => {
    ...
    const totalSuccess = Object.values(stats.success).reduce((a, b) => a + b, 0);
    const totalFail = Object.values(stats.fail).reduce((a, b) => a + b, 0);
    const totalUnsupported = Object.values(stats.unsupported).reduce((a, b) => a + b, 0);
